...
Cylinder::Cylinder(int slices, int stacks, float topRadius, float bottomRadius, float height) {
	...
	for (int i = slices; i >= 0; i--) {
		...
		vertexArray[count++] = Vertex(
				glm::vec3(topRadius * x, y, topRadius * z), glm::vec2(U, V), glm::vec3(0, y, 0));
	}
	vertexArray[count++] = Vertex(glm::vec3(0, y, 0), glm::vec2(0.5, 0.5), glm::vec3(0, y, 0));
	//bottom cap
	y = -y;
	...
	for (int i = 0; i <= slices; i++) {
		...
		float V = z / 2.0f + 0.5;
		vertexArray[count++] = Vertex(
				glm::vec3(bottomRadius * x, y, bottomRadius * z), glm::vec2(U, V), glm::vec3(0, y, 0));
	}
	vertexArray[count++] = Vertex(glm::vec3(0, y, 0), glm::vec2(0.5, 0.5), glm::vec3(0, y, 0));
