...
void init(int width, int height, std::string strTitle, bool bFullScreen) {
	...
	lightingShader.initialize("../Shaders/iluminacionMateriales.vs", "../Shaders/iluminacionMateriales.fs");
...
void applicationLoop() {
	...
	while (psi) {
		...
		GLint lightSpecularLoc = lightingShader.getUniformLocation(
			"light.specular");
		GLint lightPosLoc = lightingShader.getUniformLocation(
			"light.position");

		glUniform3f(lightAmbientLoc, 0.2f, 0.2f, 0.2f);
		glUniform3f(lightDiffuseLoc, 0.5f, 0.5f, 0.5f); // Let's darken the light a bit to fit the scene
		glUniform3f(lightSpecularLoc, 1.0f, 0.1f, 1.0f);
