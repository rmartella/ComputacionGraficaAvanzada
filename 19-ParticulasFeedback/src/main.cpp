...
// Descomentar
GLuint textureID1, textureID2, textureParticle, textureCespedID, textureWaterID, textureWindowID, textureParticleFire, texId;
GLuint skyboxTextureID;

// Definition for the particle system fountain
...
void initParticleBuffersFire() {
	...
	// Generate the buffers
	glGenBuffers(1, &initVelFire);   // Initial velocity buffer
	glGenBuffers(1, &startTimeFire); // Start time buffer

	// Allocate space for all buffers
	int size = nParticles * 3 * sizeof(float);
	glBindBuffer(GL_ARRAY_BUFFER, initVelFire);
	glBufferData(GL_ARRAY_BUFFER, size, NULL, GL_STATIC_DRAW);
	glBindBuffer(GL_ARRAY_BUFFER, startTimeFire);
	glBufferData(GL_ARRAY_BUFFER, nParticlesFire * sizeof(float), NULL, GL_STATIC_DRAW);
	...
	glBindBuffer(GL_ARRAY_BUFFER, startTimeFire);
	glBufferSubData(GL_ARRAY_BUFFER, 0, nParticlesFire * sizeof(float), data);
...
void init(int width, int height, std::string strTitle, bool bFullScreen) {
	...
	Texture texture7("../Textures/fire.png");
	bitmap = texture7.loadImage();
	data = texture7.convertToData(bitmap, imageWidth, imageHeight);
	glGenTextures(1, &textureParticleFire);
	glBindTexture(GL_TEXTURE_2D, textureParticleFire);
	// set the texture wrapping parameters
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);// set texture wrapping to GL_REPEAT (default wrapping method)
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
	// set texture filtering parameters
...
void applicationLoop() {
	...
	std::vector<glm::vec3> blendingUnsorted = { glm::vec3(3, 0.0, 15.0), glm::vec3(3.0, 0.0,
			-4.0), glm::vec3(-2.0, 0.0, -3.0), glm::vec3(-2.0, 0.0, -10.0), glm::vec3(0.0, 0.0, 0.0), glm::vec3(0.0, 0.0, 0.0)};
	...
	while (psi) {
		...
		blendingUnsorted[5] = modelMatrixColliderAircraft[3];
