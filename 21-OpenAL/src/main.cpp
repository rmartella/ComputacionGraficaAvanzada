...
void init(int width, int height, std::string strTitle, bool bFullScreen) {
	...
	buffer[0] = alutCreateBufferFromFile("../sounds/silbato.wav");
...
void applicationLoop() {
	...
	while (psi) {
		 ...
		 * Open AL sound data
		 */
		source0Pos[0] = matrixModelAirCraft[3].x;
		source0Pos[1] = matrixModelAirCraft[3].y;
		source0Pos[2] = matrixModelAirCraft[3].z;
		alSourcefv(source[0], AL_POSITION, source0Pos);

		// Listener for the Thris person camera
		listenerPos[0] = modelMatrixDart[3].x;
		listenerPos[1] = modelMatrixDart[3].y;
		listenerPos[2] = modelMatrixDart[3].z;
		alListenerfv(AL_POSITION, listenerPos);

		glm::vec3 upModel = glm::normalize(modelMatrixDart[1]);
		glm::vec3 frontModel = glm::normalize(-modelMatrixDart[0]);
