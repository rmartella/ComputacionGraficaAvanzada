...
vec3 calculateDirectionalLight(Light light, vec3 direction){
	vec4 textureColor1 = texture(texture1, our_uv);
	vec4 textureColor2 = texture(texture2, our_uv);
	vec4 textureMix = mix(textureColor1, textureColor2, 0.5);//5 porciento del color 1 y 20 porciento del color 2 (con 0.2)
	// Ambient
    vec3 ambient  = light.ambient * vec3(textureMix);//vec3(texture(texture1, our_uv))
  	
    // Diffuse 
    vec3 normal = normalize(our_normal);
    vec3 lightDir = normalize(-direction);
    float diff = max(dot(normal, lightDir), 0.0);
    vec3 diffuse  = light.diffuse * (diff * vec3(textureMix));//vec3(texture(texture1, our_uv))
    ...
    vec3 specular = light.specular * (spec * vec3(textureMix));  //vec3(texture(texture1, our_uv))
