...
Shader shaderParticlesFire; //Agregar: Shader para las particulas de fuego
...
GLuint textureParticleFountainID, textureParticleFireID, texId; //Agregar: Textura de la imagen(fire) y textura para velocidades y posiciones iniciales
...
std::map<std::string, glm::vec3> blendingUnsorted = {
		...
		{"fire", glm::vec3(0.0, 0.0, 7.0)} //Agregar posición de las particulas de fuego
};
...
GLuint feedback[2];
GLuint drawBuf = 1;
float particleSize = 0.5, particleLifetime = 10.0;//Agregar ancho de particula y tiempo de vida
...
void initParticleBuffersFire() { //Agregar: Inicialización de las particulas
	// Generate the buffers
	...
	float rate = particleLifetime / nParticlesFire; //Agregar: De esto depende el tiempo de vida de cada particula
...
void init(int width, int height, std::string strTitle, bool bFullScreen) {
	...
	shaderParticlesFire.initialize("../Shaders/particlesFire.vs", "../Shaders/particlesFire.fs", {"Position", "Velocity", "Age"}); //Agregar: Inicialización de las particulas de fuego
	...
	Texture textureParticleFire("../Textures/smoke.png"); //Agregar: Abrimos la textura que vamos a utilizar para el fuego
	bitmap = textureParticleFire.loadImage(); //Agregar: Enlazamos la imagen
	 ...
	 *******************************************/
	initParticleBuffersFire();//Agregar Hace llamado a la función
}
...
void destroy() {
	...
	shaderParticlesFire.destroy();//Destruimos el shader de la particula de fuego
	...
	glBindBuffer(GL_ARRAY_BUFFER, 0);//Agregar todas las eliminaciones de los buffer
...
void applicationLoop() {
	...
	currTimeParticlesAnimationFire = lastTime;//Agregar Obtenemos el tiempo actial
	lastTimeParticlesAnimationFire = lastTime;

	while (psi) {
		currTime = TimeManager::Instance().GetTime();
		if(currTime - lastTime < 0.016666667){ //Agregar: Para cambiar el número de frames
			glfwPollEvents();
			continue;
		}
		lastTime = currTime;
		...
		shaderParticlesFire.setMatrix4("projection", 1, false, glm::value_ptr(projection)); //Agregar Matriz de vista
		shaderParticlesFire.setMatrix4("view", 1, false, glm::value_ptr(view));//Agregar Matriz de proyección
